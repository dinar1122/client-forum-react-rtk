...
import { Card as CardNextUI, CardHeader, CardBody, Divider, Avatar, Button } from '@nextui-org/react';
...
import { subscribedCategoryNTopics, subscribedData, userFollows } from '../../utils/subscribed-data';
...
const CurrentTopic = () => {
  ...
  const {
    ...
    description,
    createdAt,
    author, } = data?.topicInfo;

    const isSubscribedOnUser = currentUserFollows?.some(user=> user.followingId === author.id)
    const isSubscribedOnTopic = subscribedCategoryNTopicsData?.topic?.some(topic=> topic.topicId === id)
  ...
  return (
    <div>
      ...
      />
      {
        data.posts.length > 0 ? data.posts.map((postData: any) => {
          return <Card
            key={postData.id}
          ...
          <CardBody className='justify-center flex-row bg-gray-100 rounded-xl'>
            <span className='w-max font-bold text-2xl p-2 text-gray-600 '>Записей нет</span>
            <Link className='w-min my-auto' to={`/create`}>
              <Button >
                <span className=' m-auto font-bold text-2xl  text-gray-600 '>Создать</span>
              </Button>
            </Link>
          </CardBody>
        </CardNextUI>
