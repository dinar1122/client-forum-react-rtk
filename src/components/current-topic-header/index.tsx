...
const CurrentTopicHeader = ({
  ...
  description,
  author,
  isSubscribedOnTopic = false,
  isSubscribedOnUser = false,
}: CurrentTopicHeaderProps) => {
  const { handleSubscribeUser, handleSubscribeTopic } =
      ...
      <Card className="mb-3 shadow-sm border">
        <CardHeader className="flex-col items-center gap-3 bg-gray-100 p-3 w-full ">
          <div className="flex-row gap-2 w-full  justify-between">
            ...
            <span className=" px-2 pt-1 text-gray-500">
              создана: {formatToClientDate(createdAt)}
            </span>
            <Button
              onClick={() => handleSubscribeTopic(isSubscribedOnTopic, id)}
              className="my-auto bg-white font-semibold text-gray-700"
              size="sm"
            >
              {isSubscribedOnTopic ? `Отписаться` : `Подписаться`}
            </Button>
          </div>
        </CardHeader>

        <CardBody>
          <p className="text-gray-500 bg-gray-100 rounded-xl p-3 w-full">
            {description}
          </p>
        </CardBody>
        <Divider></Divider>
        <CardBody className="flex-row justify-between pt-0 ">
          ...
          <Button
            onClick={() => handleSubscribeUser(isSubscribedOnUser, author.id)}
            variant="ghost"
            className="my-auto font-semibold text-gray-700"
          >
            {isSubscribedOnUser ? `Отписаться` : `Подписаться`}
          </Button>
        </CardBody>
      </Card>
    </>
