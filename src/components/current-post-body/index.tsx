  ...
  return (
    <div className="p-2 bg-gray-100 rounded-xl">
      ...
      <div className="flex justify-end mt-2">
        ...
        <Button className="mx-1" onClick={onCancel}>
          Отмена
        </Button>
      </div>
    </div>
...
const CurrentPostBody = ({
  content,
  editor = false,
  onUpdateContent = null,
  spoilerTitle = 'Спойлер',
}: CurrentPostBodyProps) => {
  ...
  const renderBlock = (
    ...
    isEditing: any,
    isShowed: any,
    shouldUseSpoiler: any,
  ) => (
    <div
      key={index}
      className={`flex flex-col py-2 px-1 myTextClass ${editor && `hover:bg-blue-100 rounded-xl transition duration-250 ease-in-out`}`}
    >
      {isEditing ? (
        <EditForm
          item={item}
        ...
        <>
          ...
          {![
            ...
            'BlockCode',
            'RePost',
          ].includes(item.component) && <TextContent>{item.text}</TextContent>}
          {editor && (
            <div className="flex justify-end items-center mt-2 ">
              <div className="flex items-center ml-2">
                <Button
                  ...
                  isIconOnly
                  onClick={() => toggleShowed(index, item)}
                >
                ...
                <Button
                  className="my-auto mx-1"
                  isIconOnly
                  onClick={() => toggleSpoiler(index, item)}
                >
                ...
                <Button
                  ...
                  isDisabled={index === content.length - 1}
                >
                  <HiOutlineArrowDown />
                </Button>
                <Button
