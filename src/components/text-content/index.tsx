...
export const TextContent: React.FC<Props> = ({
  ...
  const renderPart = (part: any, index: number) => {
    ...
    if (part.type === 'quote') {
      return (
        <blockquote
          key={index}
          className="border-l-4 border-blue-500 pl-4 italic my-2 bg-gray-200 rounded-md"
        >
          {part.display}
        </blockquote>
      );
  ...
  return (
    <div className={`${size} my-auto`}>
      {parsedText.map((part: any, index: any) => renderPart(part, index))}
    </div>
  );
};
